...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--n_forward", type=int, default=5, help="number for forward passing."
    )
    parser.add_argument(
        "--resolution",
        type=int,
    ...
    parser.add_argument("--T_in", type=int, default=1, help="Number of input views")
    parser.add_argument("--T_out", type=int, default=1, help="Number of output views")
    parser.add_argument(
        "--guidance_scale",
        type=float,
        default=3.0,
    ...
    parser.add_argument(
        ...
        type=str,
        default="6DoF",
        help=("The camera pose encoding CaPE type. Chosen from 4DoF, 6DoF"),
    )
    parser.add_argument(
        "--output_dir",
        type=str,
...
def main(args):
    ...
    for obj_name in tqdm(obj_names):
        ...
        if DATA_TYPE == "NeRF":
            ...
            # load train info
            with open(
                os.path.join(DATA_DIR, obj_name, "transforms_train.json"), "r"
            ) as f:
                train_info = json.load(f)["frames"]
            # load test info
            with open(
                os.path.join(DATA_DIR, obj_name, "transforms_test.json"), "r"
            ) as f:
            ...
            train_index = np.load(
                os.path.join(info_dir, f"train_N{T_in}M20_random.npy")
            )
            test_index = np.arange(len(test_info))  # use all test views
        elif DATA_TYPE == "Franka":
            angles_in = np.load(
                os.path.join(DATA_DIR, obj_name, "angles.npy")
        ...
        for T_in_index in train_index:
            if DATA_TYPE == "RTMV":
                ...
                img = Image.fromarray(np.uint8(img[:, :, :3] * 255.0)).convert("RGB")
                ...
                pose_in.append(
                    get_pose(
                        np.concatenate(
                            [input_RT[:3, :], np.array([[0, 0, 0, 1]])], axis=0
                        )
                    )
                )
            else:
                if DATA_TYPE == "NeRF":
                    img_path = os.path.join(
                        DATA_DIR, obj_name, train_info[T_in_index]["file_path"] + ".png"
                    )
                    ...
                    if CaPE_TYPE == "6DoF":
                        ...
                        pose[:3, 3] *= 1.0 / max_t * radius
                    elif CaPE_TYPE == "4DoF":
                        pose = np.linalg.inv(pose)
                    pose_in.append(torch.from_numpy(get_pose(pose)))
                elif DATA_TYPE == "Franka":
                    img_path = os.path.join(
                        DATA_DIR, obj_name, "images_rgba", f"frame{T_in_index:06d}.png"
                    )
                    ...
                    if CaPE_TYPE == "4DoF":
                        pose_in.append(
                            torch.from_numpy(
                                [
                                    np.deg2rad(90.0 - elevation),
                                    np.deg2rad(azimuth - 180),
                                    0.0,
                                    0.0,
                                ]
                            )
                        ...
                        neg_theta = np.deg2rad(90.0 - elevation)
                        xyz = (
                            np.array(
                                [
                                    np.sin(neg_theta) * np.cos(neg_i),
                    ...
                    if DATA_TYPE == "MVDream":
                        img_path = os.path.join(
                            DATA_DIR, obj_name, f"{T_in_index}_rgba.png"
                        )
                    elif DATA_TYPE == "Text2Img":
                    ...
                    if CaPE_TYPE == "4DoF":
                        pose_in.append(
                            torch.tensor(
                                [np.deg2rad(polar), np.deg2rad(azimuth), 0.0, 0.0]
                            )
                        )
                    elif CaPE_TYPE == "6DoF":
                    ...
                    img_path = os.path.join(
                        ...
                        obj_name,
                        T_in_DATA_TYPE,
                        "model/%03d.png" % T_in_index,
                    )
                    pose_path = os.path.join(
                        DATA_DIR,
                        obj_name,
                        T_in_DATA_TYPE,
                        "model/%03d.npy" % T_in_index,
                    )
                    pose_in.append(
                        get_pose(
                            np.concatenate(
                                [np.load(pose_path)[:3, :], np.array([[0, 0, 0, 1]])],
                                axis=0,
                            )
                        )
                    )
        ...
        for T_out_index in test_index:
            if DATA_TYPE == "RTMV":
                ...
                pose_out.append(
                    get_pose(
                        np.concatenate(
                            [output_RT[:3, :], np.array([[0, 0, 0, 1]])], axis=0
                        )
                    )
                )
            else:
                if DATA_TYPE == "NeRF":
                    img_path = os.path.join(
                        DATA_DIR, obj_name, test_info[T_out_index]["file_path"] + ".png"
                    )
                    ...
                    img_path = os.path.join(
                        ...
                        obj_name,
                        T_out_DATA_TYPE,
                        "model/%03d.png" % T_out_index,
                    )
                    pose_path = os.path.join(
                        DATA_DIR,
                        obj_name,
                        T_out_DATA_TYPE,
                        "model/%03d.npy" % T_out_index,
                    )
                    if T_out_DATA_TYPE == "render_mvs_25":  # blender coordinate
                        ...
                        pose[
                            ...
                            -1
                        )  # pose out 36 is in opencv coordinate, pose in 25 is in blender coordinate
                ...
                if img_path is not None:  # sometimes don't have GT target view image
                    img = plt.imread(img_path)
                    img[img[:, :, -1] == 0.0] = bg_color
                    img = Image.fromarray(np.uint8(img[:, :, :3] * 255.0)).convert(
                        "RGB"
                    )
                    gt_image.append(image_transforms(img))
        ...
        # [B, T, C, H, W]
        input_image = (
            torch.stack(input_image, dim=0).to(device).to(weight_dtype).unsqueeze(0)
        )
        # input_image = input_image.repeat(args.n_forward, 1, 1, 1, 1)
        # import pdb
        ...
        if len(gt_image) > 0:
            gt_image = (
                torch.stack(gt_image, dim=0).to(device).to(weight_dtype).unsqueeze(0)
            )
        # [B, T, 4]
        ...
        if CaPE_TYPE == "6DoF":
            ...
            pose_out_inv = (
                torch.from_numpy(pose_out_inv).to(device).to(weight_dtype).unsqueeze(0)
            )
        ...
        for i in range(args.n_forward):
            ...
            if args.seed is None:
                ...
                generator = torch.Generator(device=device).manual_seed(2000 + i)
        ...
        rgb_unc_image = get_rgb_variance(image_list)
        # rgb_unc_image = get_rgb_error(image_list)
        # rgb_unsim_image = get_rgb_ssim(image_list)
        ...
        imsave(
            ...
            (
                (np.concatenate(input_image.permute(0, 2, 3, 1).cpu().numpy(), 1) + 1)
                / 2
                * 255
            ).astype(np.uint8),
        )
        ...
        for i in range(T_out):
            for j in range(args.n_forward):
                imsave(
                    os.path.join(output_dir, f"{i}_forward{j+1}.png"),
                    (image_list[j][i] * 255).astype(np.uint8),
                )
            imsave(
                os.path.join(output_dir, f"var_{i}.png"),
                (visualize_unc_map(rgb_unc_image[i]) * 255).astype(np.uint8),
            )
        ...
        if len(gt_image) > 0:
            imsave(
                ...
                (
                    (np.concatenate(gt_image.permute(0, 2, 3, 1).cpu().numpy(), 1) + 1)
                    / 2
                    * 255
                ).astype(np.uint8),
            )
...
def visualize_unc_map(rgb_unc, cmap=cv2.COLORMAP_JET):
    """
    depth: (H, W)
    """

    x = np.nan_to_num(rgb_unc)  # change nan to 0
    # x = 1 - np.exp(-depth_unc / 3)
    # x = np.nan_to_num(depth_unc)  # change nan to 0
    mi = 0  # get minimum positive depth (ignore background)
    ma = np.max(x)
    # else:
    #     mi, ma = minmax
...
# def get_rgb_ssim(image_list):
#     calc_ssim = torchmetrics.StructuralSimilarityIndexMeasure(
#         data_range=1.0, return_full_image=True
#     )
#     N = len(image_list)
#     pseudo_gt_image = image_list[-1, :]
#     pseudo_gt_image = einops.repeat(pseudo_gt_image, "n h w c -> k n h w c", k=2)
#     ssim_list = []
#     pseudo_gt_image = pseudo_gt_image.permute(0, 1, 4, 2, 3)
#     compare_image = image_list[:-1, :].permute(0, 1, 4, 2, 3)
#     for i in range(N - 1):
#         _, ssim_image = calc_ssim(pseudo_gt_image[i], compare_image[i])
#         ssim_list.append(ssim_image)
#         print(ssim_image.shape)
#     # print(ssim_list.shape)
#     unsim_image = 1 - torch.sum(torch.tensor(ssim_list), dim=0) / (N - 1)
#     return unsim_image.numpy()
# print(image_error_list.shape)
# uncertainty_image = np.sum(image_error_list, axis=0) / (N - 1)


def get_rgb_variance(image_list):
    image_list = np.array(image_list)
    variance = np.var(image_list, axis=0)
    variance_image = np.mean(variance, axis=-1)
    return variance_image


def get_rgb_error(image_list):
    # image_list = np.array(image_list)
    N = len(image_list)
    pseudo_gt_image = image_list[-1, :]
    image_error_list = torch.mean(
        torch.abs(image_list[:-1, :] - pseudo_gt_image), dim=-1
    )
    uncertainty_image = torch.sum(image_error_list, dim=0) / (N - 1)

    return uncertainty_image.numpy()
